...
// PLC OPC UA 엔드포인트
const endpointUrl = process.env.PLC_ENDPOINT || "opc.tcp://192.168.0.102:4840";
...
async function initPLC() {
    try {
        ...
        client.on("connection_lost", () => {
            isConnected = false;
            console.error(`[${new Date().toISOString()}] PLC 연결 끊김`);
        });

        client.on("connection_reestablished", () => {
            isConnected = true;
            console.log(`[${new Date().toISOString()}] PLC 연결 재설정됨`);
        });
        ...
        isConnected = true;
        retryCount = 0;
        console.log(`[${new Date().toISOString()}] PLC 연결됨`);

        await readAllPLCData();
    } catch (err) {
        isConnected = false;
        retryCount++;
        console.error(`[${new Date().toISOString()}] PLC 연결 오류: ${err.message}`);
        if (retryCount < MAX_RETRIES) {
            setTimeout(initPLC, retryCount * 1000);
        }
    }
}
...
async function readAllPLCData() {
    if (!isConnected || !session) {
        throw new Error("PLC 연결이 되어있지 않습니다");
    }
...
// PLC 연결 상태 점검
async function checkPLCConnection() {
    try {
        const status = {
        ...
        if (!status.connected || !status.sessionActive) {
            console.log(`[${new Date().toISOString()}] PLC 재연결 시도 중...`);
            await initPLC();
        }
    } catch (err) {
        console.error(`[${new Date().toISOString()}] PLC 상태 확인 오류: ${err.message}`);
    }
}
